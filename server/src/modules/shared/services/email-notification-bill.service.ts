...
@Injectable()
export class EmailServiceNotificationBill {
  private readonly logger = new Logger(EmailServiceNotificationBill.name);


  async sendFeeReminderEmail(params: FeeReminderEmailParams) {
    ...
    try {
      await emailUtil(parentEmail, subject, htmlContent);
