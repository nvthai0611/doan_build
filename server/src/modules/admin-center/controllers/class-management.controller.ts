...
export class ClassManagementController {
  ...
  @ApiOperation({
    summary:
      'Lấy danh sách lớp học khả dụng cho chuyển lớp (bao gồm thông tin giáo viên)',
  })
  @ApiResponse({
    status: 200,
    description: 'Danh sách lớp học với thông tin giáo viên',
  })
  async getClassesForTransfer(@Query() queryDto: QueryClassDto) {
  ...
  @ApiResponse({
    status: 400,
    description:
      'Giáo viên đã được phân công hoặc lịch học bị trùng với các lớp đã được phân công',
  })
  @ApiResponse({
    status: 404,
  ...
  async transferTeacher(
    ...
    return this.classManagementService.transferTeacher(
      classId,
      body,
      requestedBy,
    );
  ...
  async approveTransfer(
    ...
    return this.classManagementService.approveTransfer(
      transferId,
      body,
      approvedBy,
    );
  ...
  async rejectTransfer(
    ...
    return this.classManagementService.rejectTransfer(
      transferId,
      body,
      rejectedBy,
    );
