...
export class CommonController {
    ...
    async getStudentsByAssignment(@Param('assignmentId') assignmentId: string) {
        try {
            const result = await this.commonService.getListStudentOfClass(assignmentId);
            return result;
        } catch (error) {
            return {
                success: false,
    ...
    @ApiOperation({ 
        ...
        description: 'Lấy danh sách tất cả học sinh đang học trong lớp thông qua teacher class assignment ID'
    })
    @ApiParam({ 
        name: 'assignmentId', 
        description: 'ID phân công giáo viên - lớp học',
    ...
    async getDetailStudentOfClass(
        ...
        try {
            const result = await this.commonService.getDetailStudentOfClass(studentId, assignmentId);
            return result;
        } catch (error) {
            return {
                success: false,
