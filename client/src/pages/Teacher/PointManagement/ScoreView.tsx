...
export default function StudentGradesPage() {
  ...
  // Pagination state for the main grades list
  ...
  const columns: Column<StudentGradeDetail>[] = [
    {
      key: 'student',
      header: 'Học sinh',
      render: (item) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{item.studentCode}</p>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (item) => (
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Lớp {item.class}</p>
        </div>
      ),
    },
    {
      key: 'detail',
      header: 'Điểm chi tiết',
      align: 'left',
      render: (item) => (
    ...
    },
    {
      ...
      align: 'left',
      render: (item) => (
        <div className={`text-lg font-bold ${getGradeColor(item.average)}`}>{item.average}</div>
      ),
    },
    {
      key: 'classification',
        ...
        <TabsContent value="grades" className="space-y-6">
          <Card>
            ...
            <CardContent>
              ...
              {/**
               * Paginate the grades locally. The server already applies filters, so
               * we only slice the returned array for client-side paging UI.
               */}
              {
                (() => {
                  ...
                  return (
                    <DataTable
                      ...
                      loading={isLoading}
                      error={isError ? ((error as any)?.message ?? 'Lỗi tải dữ liệu') : null}
                      onRetry={refetch}
                      emptyMessage="Chưa có bản ghi điểm"
                      enableSearch={false}
                    />
                  )
                })()
