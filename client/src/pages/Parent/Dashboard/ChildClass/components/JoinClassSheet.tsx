...
import { X, Calendar, User, Clock, AlertCircle, Upload, Download } from 'lucide-react';
...
export const JoinClassSheet = ({ open, onOpenChange }: JoinClassSheetProps) => {
  ...
  const validCommitments = allCommitments.filter((commitment: any) => {
    if (!classInfo?.subject?.id) return false;
    const hasSubject = commitment.subjectIds?.includes(classInfo.subject.id);
    ...
    if (contractFile && !selectedContractId) {
      ...
      try {
        ...
        const uploadResponse = await parentCommitmentsService.uploadCommitment({
          ...
          file: contractFile,
          subjectIds: [classInfo.subject.id], // Chỉ upload cho môn học của lớp này
          note: `Hợp đồng được tạo khi đăng ký lớp ${classInfo.name}`,
        });
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
                                {allCommitments.length === 0
                                  ? `Học sinh này chưa có hợp đồng nào cho môn "${classInfo.subject?.name || ''}". Vui lòng upload hợp đồng bên dưới.`
                                  : `Không có hợp đồng nào cho môn "${classInfo.subject?.name || ''}" hoặc hợp đồng đã hết hạn. Vui lòng upload hợp đồng mới.`}
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className={`border-2 border-dashed rounded-lg p-4 hover:border-primary/50 transition-colors ${!contractFile ? 'border-red-300' : ''}`}>
                          ...
                          {!contractFile ? (
                            ...
                            <div className="space-y-3">
                              ...
                              <p className="text-xs text-muted-foreground">
                                Hợp đồng này sẽ được tạo cho môn "{classInfo.subject?.name || ''}" khi gửi yêu cầu
                              </p>
                            </div>
                          )}
                        </div>
