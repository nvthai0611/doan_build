...
interface SubstituteTeacherSheetProps {
  open: boolean
...
export const SelectSubstituteTeacherSheet = ({
  open,
  onOpenChange,
  className,
  subjectName,
  ...
  const totalPages =
    (data as any)?.meta?.totalPages || Math.max(1, Math.ceil(total / pageSize))

  const current = useMemo(() => {
    let source = apiTeachers

    // Loại bỏ giáo viên theo excludeTeacherId (giáo viên đang xin nghỉ)
    if (excludeTeacherId) {
      source = source.filter((t: any) => t.id !== excludeTeacherId)
    }
  ...
  const handleSubmit = () => {
    if (selectedTeacherId) {
      // Bảo vệ thêm: không cho submit đúng giáo viên xin nghỉ
      if (excludeTeacherId && selectedTeacherId === excludeTeacherId) {
        return
      }
              ...
              <div className="text-center">
                ...
                <p className="text-sm">
                  {query
                    ...
                    : 'Vui lòng thêm giáo viên vào hệ thống'}
                </p>
              </div>
            </div>
          ) : (
...
export default SelectSubstituteTeacherSheet
