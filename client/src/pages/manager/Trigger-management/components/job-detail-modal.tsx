    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-card/80 backdrop-blur-sm border-border/50">
        <DialogHeader className="border-b border-border/40 pb-4">
          ...
          <DialogDescription className="text-xs text-muted-foreground">Job execution ID: {job.id}</DialogDescription>
        </DialogHeader>
        ...
        <Tabs defaultValue="overview" className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-muted/30 border-b border-border/40">
            ...
            {job.errorDetails && <TabsTrigger value="errors">Errors ({job.errorDetails.length})</TabsTrigger>}
          </TabsList>
          ...
          <TabsContent value="overview" className="space-y-4 mt-4">
            <Card className="border-border/40 bg-card/50">
              ...
              <CardContent className="space-y-4 pt-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-wider">Started At</p>
                    <p className="font-medium text-sm mt-1">{formatDate(job.startedAt)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-wider">Completed At</p>
            ...
            <Card className="border-border/40 bg-card/50">
              <CardHeader className="border-b border-border/40">
                <CardTitle className="text-sm">Item Processing Summary</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 pt-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs font-medium uppercase tracking-wider flex items-center gap-2">
                    <CheckCircle2 className="h-4 w-4 text-green-400" /> Successfully Processed
                  </span>
                  <span className="font-bold text-green-400">{job.successCount}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs font-medium uppercase tracking-wider flex items-center gap-2">
                    <AlertCircle className="h-4 w-4 text-red-400" /> Failed Items
                  </span>
          ...
          {job.errorDetails && (
            <TabsContent value="errors" className="space-y-4 mt-4">
              <Card className="border-border/40 bg-card/50">
                <CardHeader className="border-b border-border/40">
                  <CardTitle className="text-sm">Failed Items ({job.errorDetails.length})</CardTitle>
                </CardHeader>
                <CardContent className="pt-4">
